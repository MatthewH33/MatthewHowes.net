...
│           ├── items.js            POST /api/classes/:code/items
│           └── items/
...
> **Why `classes.js` sits next to a `classes/` folder:** plain Vercel
> Serverless Functions (no framework) map file paths to URLs *literally* —
> unlike Next.js, `index.js` inside a folder does **not** collapse to the
> parent path. `api/classes/index.js` would have routed to
> `/api/classes/index`, not `/api/classes`. Naming the file after the route
> segment itself (`classes.js`, `items.js`) avoids that trap.
